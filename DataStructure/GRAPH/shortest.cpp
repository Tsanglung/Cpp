...
{
    int i;
    for (i = 0; i < n; i++) {s[i] = false; dist[i] = length[v][i];} // initialize
    s[v] = true;
    dist[v] = 0;
...
{
    int i, k, l, u;
    for (i = 0; i < n; i++) dist[i] = length[v][i]; // initialize @dist@
    for (k = 2 ; k <= n-1; k++)
    {
        for (l = 0; l < n; l++) newdist[l] = dist[l];
        for (u = 0; u < n ; u++)
            if (u != v) {
                for (i = 0; i < n; i++)
	                if ((u != i) && (length[i][u] < 10000))
...
// @a[i][j]@ is the length of the shortest path between @i@ and @j@
{
    ...
    for (k = 0; k < n; k++) {  // for a path with highest vertex index @k@
       OutA(n);
